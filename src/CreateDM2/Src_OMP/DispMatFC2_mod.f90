...
module DispMatFC2
    ...
    public      :: IterOver_N2, find_force

contains

    subroutine  IterOver_N2( mu, alpha, num_row, nstep, NIndFC, NumDef_sup, PointDef, &
        ...
        !$omp parallel do default(shared) &
                    ...
                    !$omp private(N2, beta, dv) &
                    !$omp shared(Mat, flags, ColLock, disp, sup_dim, FC2) &
        ...
        N2_loop: do N2 = 1, Natm
            beta_loop: do beta = 1, 3
                ...
                dv = FC2%F(1, beta, alpha, N2, mu)

                dv8_chk: if ( dabs(dv - 8.0_dp) > EPS ) then

                    call FindLC( disp, num_row, nstep, sup_dim, FC2, mu, N2, alpha, beta, &
                               & NumDef_sup, PointDef, cell_atm_def, dv, &
                               & Mat, flags, ColLock )
    ...
    end subroutine IterOver_N2


    subroutine FindLC( disp, num_row, nstep, sup_dim, FC2, mu, N2, alpha, beta, &
                     & NumDef_sup, PointDef, cell_atm_def, dv, &
                     & Mat, flags, ColLock )
        ...
        dv_chk: if ( (dabs(dv+1.0_dp) < EPS) .or. (dabs(dv+7.0_dp) < EPS) ) then
            ...
            call find_ind17(Mat, ColLock, FC2, num_row, mu, N2, alpha, beta, dsp_col)

        else if ( dabs(dv+5.0_dp) < EPS ) then dv_chk

            call find_ind5(Mat, ColLock, FC2, num_row, mu, N2, alpha, beta, dsp_col)

        else if ( dabs(dv+9.0_dp) < EPS ) then dv_chk

            call find_ind9(Mat, ColLock, FC2, num_row, mu, N2, alpha, beta, 1.0_dp, dsp_col)
    ...
    subroutine find_ind17(Mat, ColLock, FC2, num_row, mu, N2, &
                        & alpha, beta, dsp_col)
        ...
        coef_lc_loop: do jj = 1, MAX_CF
            ...
            cf0_chk: if ( dabs(cf) > EPS ) then
                ...
                dv3_chk: if ( dabs(dep_var3+9.0_dp) < EPS ) then
                    ...
                    call find_ind9(Mat, ColLock, FC2, num_row, fc_lc(1), fc_lc(2), a_lc, b_lc, cf, dsp_col)
    ...
    subroutine find_ind5(Mat, ColLock, FC2, num_row, mu, N2, &
                       & alpha, beta, dsp_col)
        ...
        coef_lc_loop: do jj = 1, MAX_CF
            ...
            cf0_chk: if ( dabs(cf) > EPS ) then
                ...
                dv2_chk: if ( dabs(dep_var2+9.0_dp) < EPS ) then
                    ...
                    call find_ind9(Mat, ColLock, FC2, num_row, mu, N2, a_lc, b_lc, cf, dsp_col)
    ...
    subroutine find_ind9(Mat, ColLock, FC2, num_row, mu, N2, &
                       & a, b, cf, dsp_col)
