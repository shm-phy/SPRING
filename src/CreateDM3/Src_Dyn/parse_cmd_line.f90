...
subroutine get_command_line(infile, T, mu, alpha, DynSchd, Nchunk, StrtImg)

    implicit none

    character(len=1024), parameter      :: expected_cmd_line="./a.out -inp [INPUT FILE] -T [TEMPERATURE] &
                                         & -mu [INTEGER] -alpha [INTEGER] -DynSchd [LOGICAL] -Nchunk [INTEGER] -IdleImg1 [LOGICAL]"
    integer, parameter                  :: MAX_CMD_ARG=14

    character(len=*), intent(out)       :: infile
    real(dp), intent(out)               :: T
    integer, intent(out)                :: mu, alpha, Nchunk, StrtImg
    logical, intent(out)                :: DynSchd
    ...
    chk_no_cmd: if ( Narg > 0 .and. Narg <= MAX_CMD_ARG ) then
        ...
        loop_args: do 
            ...
            err_chk: if ( err == 0) then
                ...
                chk_cmd_name: if ( cmd_name == "-inp" ) then
                ...
                else if ( cmd_name == "-DynSchd" ) then chk_cmd_name
                    ...
                    if ( err == 0 ) then
                        ...
                        alpha_char = cmd_arg
                        dflt_val(5) = .false.
                        read(alpha_char, *) DynSchd

                        img1_chk9: if ( this_image() == 1 ) then
                            write(*, 450) DynSchd
                            450 FORMAT('Dynamic schedule is set to:', L7)
                        end if img1_chk9
                ...
                else if ( cmd_name == "-Nchunk" ) then chk_cmd_name
                    ...
                    if ( err == 0 ) then
                        ...
                        alpha_char = cmd_arg
                        dflt_val(6) = .false.
                        read(alpha_char, *) Nchunk

                        img1_chk7: if ( this_image() == 1 ) then 
                            write(*, 700) Nchunk
                            700 FORMAT('Nchunk: ', I6)
                        end if img1_chk7
                ...
                else if ( cmd_name == "-IdleImg1" ) then chk_cmd_name
                    ...
                    if ( err == 0 ) then
                        ...
                        alpha_char = cmd_arg
                        dflt_val(7) = .false.
                        read(alpha_char, *) IdleImg1

                        img1_chk91: if ( (this_image() == 1) .and. &
                                      & (.not.(IdleImg1)) ) then

                            write(*, 50) IdleImg1
                            50 FORMAT('Idling of image 1 is set to:', L7)
        ...
        outer_if: if ( dflt_val(i) ) then
            ...
            inner_if: if ( i == 1 ) then
                ...
                img1_chk10: if ( this_image() == 1 ) then
                    write(*, 150) DynSchd
                    150 FORMAT('Dynamic Schedule is set to default:', L7)
                end if img1_chk10
                ...
                img1_chk8: if ( (this_image() == 1) .and. DynSchd ) then 
                    write(*, 750) Nchunk
                    750 FORMAT('Nchunk is set to default: ', I3)
                end if img1_chk8
...
subroutine print_err_msg(err, nn, cmd_line, expected_cmd_line)
    ...
    subroutine ShowWelcomeBanner()
        ...
        40 FORMAT(10X, "Executable: create_disp_mat3.x (Creation of 3rd-order displacement matrix)")
