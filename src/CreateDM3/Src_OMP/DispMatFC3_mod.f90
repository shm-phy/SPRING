...
contains
    ...
    subroutine  IterOver_N2_N3( mu, alpha, num_row, nstep, NIndFC, NumDef_sup, PointDef, &
                              & cell_atm_def, sup_dim, FC3, F, disp, flags, Mat )
        ...
        !$omp parallel do default(shared) &
                    ...
                    !$omp private(N2, N3, beta, gama, dv) &
                    !$omp shared(Mat, flags, ColLock, disp, sup_dim, FC3, F) &
                    !$omp shared(mu, alpha, Natm, num_row, nstep, chunk) &
                ...
                beta_loop: do beta = 1, 3
                    gama_loop: do gama = 1, 3
                        ...
                        dv8_chk: if ( dabs(dv - 8.0_dp) > EPS ) then
                            ...
                            call FindLC( disp, num_row, nstep, sup_dim, FC3, F, mu, N2, N3, &
                                       & alpha, beta, gama, NumDef_sup, PointDef, cell_atm_def, &
    ...
    subroutine FindLC( disp, num_row, nstep, sup_dim, FC3, F, mu, N2, N3, &
                     & alpha, beta, gama, NumDef_sup, PointDef, cell_atm_def, &
                     & dv, Mat, flags, ColLock )
        ...
        call find_disp_coeff( FC3, sup_dim, nstep, num_row, mu, N2, beta, &
                            & N3, gama, NumDef_sup, PointDef, cell_atm_def, disp, flags, dsp_col )

        dv_chk: if ( (dabs(dv+1.0_dp) < EPS) .or. (dabs(dv+7.0_dp) < EPS) ) then

            call find_ind17(Mat, ColLock, FC3, F, num_row, mu, N2, N3, alpha, beta, gama, dsp_col)

        else if ( dabs(dv+5.0_dp) < EPS ) then dv_chk

            call find_ind5(Mat, ColLock, FC3, F, num_row, mu, N2, N3, alpha, beta, gama, dsp_col)

        else if ( dabs(dv+9.0_dp) < EPS ) then dv_chk

            call find_ind9(Mat, ColLock, FC3, num_row, mu, N2, N3, alpha, beta, gama, 1.0_dp, dsp_col)
    ...
    subroutine find_ind17(Mat, ColLock, FC3, F, num_row, mu, N2, N3, &
        ...
        integer(omp_lock_kind), dimension(:), intent(inout)     :: ColLock
        ...
        coef_lc_loop: do jj = 1, MAX_CF
            ...
            cf0_chk: if ( dabs(cf) > EPS ) then
                ...
                dv3_chk: if ( dabs(dep_var3+9.0_dp) < EPS ) then
                    ...
                    call find_ind9(Mat, ColLock, FC3, num_row, &
    ...
    subroutine find_ind5(Mat, ColLock, FC3, F, num_row, mu, N2, N3, &
                       & alpha, beta, gama, dsp_col)
        ...
        coef_lc_loop: do jj = 1, MAX_CF
            ...
            cf0_chk: if ( dabs(cf) > EPS ) then
                ...
                dv2_chk: if ( dabs(dep_var2+9.0_dp) < EPS ) then
                    ...
                    call find_ind9(Mat, ColLock, FC3, num_row, mu, N2, N3, a_lc, b_lc, c_lc, cf, dsp_col)
    ...
    subroutine find_ind9(Mat, ColLock, FC3, num_row, mu, N2, N3, &
                       & a, b, c, cf, dsp_col)
