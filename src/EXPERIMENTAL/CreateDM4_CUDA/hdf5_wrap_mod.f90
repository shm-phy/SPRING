...
module hdf5_wrap
    ...
    implicit none
    private
    public :: r_force_disp, wFD4_part, wFD_JoinAll
...
subroutine wFD_JoinAll(Nbasis, num_row, NIndFC, Temp_char, prefix, file_full)
    ...
    character(len=*), intent(in)                            :: Temp_char, prefix, file_full
    ...
    write(*, 225) 
    225 FORMAT('Combining Displacement-Force data ...')
    ...
    mu_loop: do mu = 1, Nbasis
        alpha_loop: do alpha = 1, 3
            ...
            file_part = 'FD_4th_mu'//trim(adjustl(adjustr(mu_char)))//'_a'//trim(adjustl(adjustr(alpha_char)))&
                      &//'_'//trim(prefix)//trim(adjustl(adjustr(Temp_char)))//'K.h5'

            call h5fopen_f(file_part, H5F_ACC_RDONLY_F, file_id, h5err) !output -> file_id
