...
subroutine Extend_Mat(this)
    ...
    allocated_chk: if ( .not. (allocated(this%InterMat)) ) then
        ...
        NumRow = this%ind_f
        NumCol = (this%NumInterPer + this%NumInterPnt) * (3**ordfc2)
        ...
        NumCol = InterMatCol + ( (this%NumInterPer + this%NumInterPnt) * (3**ordfc2) )
...
subroutine CreateInterGmat(this, mu_dep, dep_N2, dep_var, &
    ...
    integer                                     :: a, b
    integer, dimension(ordfc2)                  :: dep_cart
    ...
    a_loop: do a = 1, 3
        b_loop: do b = 1, 3
            ...
            dep_var_chk: if ( dep_var == -7 ) then
                call this%LeftSidePer(mu_ind, N2_ind, &
                                    & a, b, comb, dep_cart, row)
            else dep_var_chk
                call this%LeftSidePntGr(mu_ind, N2_ind, &
                                      & a, b, mat_num, PntPer, dep_cart, row)
            end if dep_var_chk

            call this%RightSide(mu_dep, dep_N2, dep_cart(1), dep_cart(2), row)
...
subroutine LeftSidePer(this, mu_ind, N2_ind, &
                     & a, b, comb, dep_cart, row)
    ...
    integer, dimension(ordfc2), intent(out)     :: dep_cart
    real(dp), dimension(:), intent(inout)       :: row

    ! ===================================== Local Variables ===================================== !

    integer, dimension(ordfc2)                  :: ind_cart
    integer                                     :: af, i_ind
    ...
    call this%Find_Lin_Dep(mu_ind, N2_ind, coef, row)

end subroutine LeftSidePer


subroutine LeftSidePntGr(this, mu_ind, N2_ind, &
                       & a, b, mat_num, PntPer, dep_cart, row)
    ...
    coef = -1.0_dp * outkron9_1 !important -1
...
subroutine RightSide(this, mu_dep, dep_N2, &
                   & ar, br, row)
    ...
    dvar_tst = this%F(mu_dep)%FCmu(1, br, ar, dep_N2)
    ...
    ind_Indx = int( this%F(mu_dep)%FCmu(3:4, br, ar, dep_N2) )

    coef = this%F(mu_dep)%FCmu(5:, br, ar, dep_N2)

    call this%Find_Lin_Dep(ind_Indx(1), ind_Indx(2), coef, row)
...
subroutine Find_lin_Dep(this, mu_ind, ind_N2, coef, row)
    ...
    real(dp)                                    :: dep_var1, cf
    integer                                     :: a_ind, b_ind, colno
    integer                                     :: i, ip

    ! ==================================== Local Variales ==================================== !

    coef_loop: do i = 1, (3**ordfc2)
        ...
        non_zero: if ( dabs(cf) > EPS ) then
            ...
            dep_var1 = this%F(mu_ind)%FCmu(1, b_ind, a_ind, ind_N2)

            dep_var_chk: if ( dabs(dep_var1-1.0_dp) < EPS_nzero ) then

                call this%find_col_ind(mu_ind, ind_N2, a_ind, b_ind, colno)
                row(colno) = row(colno) + cf

            else if ( dabs(dep_var1+5.0_dp) < EPS_nzero ) then dep_var_chk

                call this%find_ind5(mu_ind, ind_N2, a_ind, b_ind, cf, row)
...
subroutine find_ind5(this, mu, N2, a, b, cof, row)

    implicit none
    class(FCtyp)                                :: this

    integer, intent(in)                         :: mu, N2, a, b
    ...
    coef_dep = this%F(mu)%FCmu(5:, b, a, N2)
    ...
    coef_depLoop: do i = 1, (3**ordfc2)
        ...
        nonZeroChk: if ( dabs(cf) > EPS ) then
            ...
            call this%find_col_ind(mu, N2, a_ind, b_ind, colno)
...
subroutine find_col_ind(this, mu, N2, a_ind, b_ind, colno)

    implicit none
    class(FCtyp)                                :: this

    integer, intent(in)                         :: mu, N2, a_ind, b_ind
    ...
    dep_var1 = this%F(mu)%FCmu(1, b_ind, a_ind, N2)

    depVarChk: if ( dabs(dep_var1-1.0_dp) < EPS_nzero ) then

        colno = this%IndFCPos(mu)%Pos_mu(b_ind, a_ind, N2)
