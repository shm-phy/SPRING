...
subroutine ReducePntPerASR(this, Nbasis, ZeroEPS, Cut_Off)
    ...
    integer                                 :: numfc
    integer, dimension(2*ordfc2)            :: FCindx
    ...
    mu_loop: do mu = 1, Nbasis
        ...
        allocate( this%PosIndx_fin(mu)%Pos_mu(3,3,Natm), STAT=istat, ERRMSG=msg )
        if ( istat /= 0 ) call this%AllocationError( istat, msg )
    ...
    allocate( this%PosIndx_flat_fin( (2*ordfc2), numfc ) )
    ...
    fc_loop: do nf = numfc, 1, -1
        ...
        PivotChk: if ( dabs(pivot) > ZeroEPS ) then
            ...
            AftrChk: if ( (size(after) == 0) .or. (all(dabs(after) < ZeroEPS)) ) then
                ...
                this%F(FCindx(1))%FCmu(1, FCindx(4), FCindx(3), FCindx(2)) = 8.0_dp
                ...
                this%F(FCindx(1))%FCmu(1, FCindx(4), FCindx(3), FCindx(2)) = -9.0_dp
    ...
    ResizePosIndxFlat: if ( this%PosIndCntr_fin == this%Indfin ) then
        ...
        allocate( this%PosIndx_flat_fin((2*ordfc2), NumCol) )
    ...
    80 FORMAT("The FC corresponfing to the indx: [", 3(I3, ', '), I3, "] is zero.")
...
subroutine MakeIndIndxFin(this, FCindx)
    ...
    integer, dimension(2*ordfc2), intent(in)    :: FCindx
    ...
    val = this%PosIndx_fin(FCindx(1))%Pos_mu(FCindx(4), FCindx(3), FCindx(2)) 
    ...
    ValChk: if ( val /= FILLVAL ) then
        ...
        this%PosIndx_fin(FCindx(1))%Pos_mu(FCindx(4), FCindx(3), FCindx(2)) = this%PosIndCntr_fin 
