...
module FCmod
    ...
    use kinds,              only : dp
    use constants,          only : EPS_nzero, ordfc3, &
...
contains
    ...
    subroutine init_FCtyp(this, ChnkSz, AdvncOut, sys, Cut_Off, PntPer)
        ...
        cnst = 2+ordfc3+(3**ordfc3)

        this%TotalFCNum = 0
        mu_loop1: do mu = 1, Nbasis

            Natm = Cut_Off%numAtom(mu)
            allocate( this%F(mu)%FCmu(cnst,3,3,3,Natm,Natm) )

            this%F(mu)%FCmu = 0.0_dp

            this%TotalFCNum = this%TotalFCNum + ((Natm**2) * (3**ordfc3))
        ...
        mu_loop2: do mu = 1, Nbasis
            ...
            Natm = Cut_Off%numAtom(mu)
            allocate( this%IndFCPos(mu)%Pos_mu(3,3,3,Natm,Natm) )
        ...
        this%PerInter = [ (i, i=2,6) ]

        allocate( this%PntGrIter( NSymApply ) )
        this%PntGrIter = [ (i, i=2,PntPer%Nsym) ]
        this%PerIter = [ (i, i=2,6) ]

        this%Pos_flat_size = initMatsz2
        this%PosIndCntr = 0
        allocate( this%PosIndx_flat( (2*ordfc3), this%Pos_flat_size ) )
        this%PosIndx_flat = 0

        this%InterMatChunk = 0
        this%InterMatPos = 0
        this%InterMatChnkSize = ChnkSz ! in MB
        this%InterMatFile = 'InterMatChunk_3rd_'//trim(sys%prefix)//'.h5'
    ...
    subroutine Find_Independent(this, sys, Cut_Off, PntPer)
        ...
        mu_loop: do mu = 1, Nbasis
            ...
            N2_loop: do N2 = 1, Natm
                N3_loop: do N3 = 1, Natm
                    ...
                    call this%PrimitiveExists(mu, N2, N3, PrmtvExst)

                    apply_pntper: if (.not. PrmtvExst) then

                        call this%ApplySymm(mu, N2, N3, PntPer, Cut_Off, sys)
        ...
        ResizePosIndxFlat: if ( this%PosIndCntr == this%ind_f ) then
            ...
            allocate( this%PosIndx_flat((2*ordfc3), NumCol) )
    ...
    subroutine PrimitiveExists(this, mu, N2, N3, PrmtvExst)
        ...
        else if ( all(dabs(this%F(mu)%FCmu(2,:,:,:,N3,N2)) < EPS) ) then prmtv_chk
            PrmtvExst = .false.

        else if ( all(dabs(this%F(mu)%FCmu(2,:,:,:,N3,N2) - 1.0_dp) < EPS) &
           & .or. all(dabs(this%F(mu)%FCmu(2,:,:,:,N3,N2) + 1.0_dp) < EPS) &
           & .or. all(dabs(this%F(mu)%FCmu(2,:,:,:,N3,N2) + 7.0_dp) < EPS) ) then prmtv_chk
    ...
    subroutine ApplySymm(this, mu_ind, N2_ind, N3_ind, PntPer, Cut_Off, sys)
        ...
        call this%Fillup_cartesian(mu_ind, N2_ind, N3_ind, 1)
        ...
        this%NumInterPer = 0
        !this%PerInter(:) = 0
        call this%Permute(mu_ind, N2_ind, N3_ind, &
                        & writetype2GMat, InterGMat, PntPer, Cut_Off)

        this%NumInterPnt = 0
        !this%PntGrInter(:) = 0
        call this%PointGroup(mu_ind, N2_ind, N3_ind, &
                           & writetype2GMat, InterGMat, PntPer, Cut_Off, sys)

        gaus_el_chk: if ( this%IdenCount /= 0 ) then

            call this%cart_dep(mu_ind, N2_ind, N3_ind)
        ...
        call this%Create_Index_ind(mu_ind, N2_ind, N3_ind)
    ...
    subroutine Fillup_cartesian(this, mu_ind, N2_ind, N3_ind, dep_var)

        implicit none

        class(FCtyp)                                :: this

        integer, intent(in)                         :: mu_ind, N2_ind, N3_ind, dep_var
        ...
        ind_Indx = (/mu_ind, N2_ind, N3_ind/)

        alpha_loop: do alpha = 1, 3
            beta_loop: do beta = 1, 3
                gama_loop: do gama = 1, 3

                     this%F(mu_ind)%FCmu(1:2, gama, beta, alpha, N3_ind, N2_ind) = dble( dep_var )
                     this%F(mu_ind)%FCmu(3:5, gama, beta, alpha, N3_ind, N2_ind) = dble( ind_Indx )

                     coef = 0.0_dp
                     i_ind = 9*(alpha-1) + 3*(beta-1) + gama !- 1) + 1
                     coef(i_ind) = coef(i_ind) + 1.0_dp

                     this%F(mu_ind)%FCmu(6:, gama, beta, alpha, N3_ind, N2_ind) = coef
    ...
    subroutine Permute(this, mu_ind, N2_ind, N3_ind, &
                     & writetype2GMat, InterGMat, PntPer, Cut_Off)
        ...
        integer, dimension(5)                       :: PerIndx
        integer, dimension(ordfc3)                  :: comb
        integer, dimension(4, ordfc3)               :: ind_array

        integer, dimension(4)                       :: dep_N1crt_mu, dep_N2crt_nu, &
                                                     & dep_N3crt_et

        integer                                     :: mu_dep, dep_N2, dep_N3
        ...
        chk_interGMat: if ( InterGMat ) then
            ...
            NumPerIndx = size( PntPer%Per, 2) - 1 !5
        ...
        perindx_loop: do i = 1, NumPerIndx
            ...
            out_chk: if ( .not. outside ) then
                ...
                same_map: if ( (mu_dep == mu_ind) .and. (dep_N2 == N2_ind) .and. &
                    ...
                    call this%Exists(mu_dep, dep_N2, dep_N3, -7, &
                                   & mu_ind, N2_ind, N3_ind, comb, indx, &
                    ...
                    fillupdep_Per: if ( (.not. exist_chk) .and. (.not. InterGMat) ) then
                        ...
                        call this%Fillup_cart_dep_perm(mu_dep, dep_N2, dep_N3, -7, &
                                                     & mu_ind, N2_ind, N3_ind, comb)
    ...
    subroutine MakeGaussMatTyp3(this, comb)
        ...
        integer, dimension(ordfc3)                  :: ind_cart, dep_cart
        real(dp), dimension(3**ordfc3, 3**ordfc3)   :: GaussMatloc
        real(dp), dimension(3**ordfc3)              :: coef
        ...
        alpha_loop: do alpha = 1, 3
            beta_loop: do beta = 1, 3
                gama_loop: do gama = 1, 3
                    ...
                    ind_cart = (/alpha, beta, gama/)

                    atm4_loop: do af = 1, ordfc3
                    ...
                    i_dep = 9*(dep_cart(1) - 1) + 3*(dep_cart(2) -1) + dep_cart(3) !-1 + 1
        ...
        gaussMatExist: if ( this%IdenCount == 0) then
            allocate( this%GaussElMat(3**ordfc3, 3**ordfc3) )
            ...
            allocate( GaussMatcopy( 3**ordfc3, Matsize2 ) )
            GaussMatcopy = this%GaussElMat
            deallocate( this%GaussElMat )

            allocate( this%GaussElMat( 3**ordfc3, (Matsize2+3**ordfc3) ) )
    ...
    subroutine Exists(this, mu_dep, dep_N2, dep_N3, dep_var, &
                          & mu_ind, N2_ind, N3_ind, comb, mat_num, &
        ...
        real(dp)                                    :: var
        integer, dimension(ordfc3)                  :: ind_Indx, indx_chk

        ! ====================================== Local Variables ====================================== !

        var = this%F(mu_dep)%FCmu(2, 1, 1, 1, dep_N3, dep_N2)
        indx_chk = int( this%F(mu_dep)%FCmu(3:5, 1, 1, 1, dep_N3, dep_N2) )
                    ...
                    inner_if2: if ( dabs(var-dble(dep_var)) < EPS ) then
                        ...
                        inner_if3: if ( .not. InterGMat ) then
                            ...
                            inner_if4: if (writetype2GMat) then
                                ...
                                call this%MakeGaussMatTyp4(mu_dep, dep_N2, dep_N3, comb)
                    ...
                    iner2_if2: if ( ( dabs(var-dble(dep_var)) < EPS ) .or. &
                        ...
                        iner2_if3: if ( .not. InterGMat ) then
                            ...
                            iner2_if4: if (writetype2GMat) then
                                ...
                                call this%MakeGaussMatTyp2(mu_dep, dep_N2, dep_N3, mat_num, PntPer)
            ...
            !else if ( .not. ( all((indx_chk-ind_Indx) == 0) ) ) then out2_if
                ...
                iner3_if1: if ( ( dabs(var+1.0_dp) < EPS ) .or. &
                    ...
                    iner3_if2: if ( .not. InterGMat ) then
                        ...
                        call this%CreateInterGmat(mu_dep, dep_N2, dep_N3, dep_var, &
    ...
    subroutine Fillup_cart_dep_perm(this, mu_dep, dep_N2, dep_N3, dep_var, &
                                        & mu_ind, N2_ind, N3_ind, comb)
        ...
        alpha_loop: do alpha = 1, 3
            beta_loop: do beta = 1, 3
                gama_loop: do gama = 1, 3
                     ...
                     this%F(mu_dep)%FCmu(1:2, dep_cart(3), dep_cart(2), dep_cart(1), &
                                            & dep_N3, dep_N2) = dble( dep_var )

                     this%F(mu_dep)%FCmu(3:5, dep_cart(3), dep_cart(2), dep_cart(1), &
                                            & dep_N3, dep_N2) = dble( ind_Indx )

                     this%F(mu_dep)%FCmu(6:, dep_cart(3), dep_cart(2), dep_cart(1), &
                                           & dep_N3, dep_N2) = coef
    ...
    subroutine MakeGaussMatTyp4(this, mu_dep, dep_N2, dep_N3, comb)
        ...
        integer, dimension(ordfc3)                  :: ind_cart, dep_cart
        real(dp), dimension(3**ordfc3)              :: coef, coef1
        ...
        alpha_loop: do alpha = 1, 3
            beta_loop: do beta = 1, 3
                gama_loop: do gama = 1, 3
                    ...
                    coef1 = this%F(mu_dep)%FCmu(6:, dep_cart(3), dep_cart(2), dep_cart(1), &
                                                  & dep_N3, dep_N2)

                    coef = (coef - coef1)

                    i = 9*(alpha-1) + 3*(beta-1) + gama !- 1) + 1
    ...
    subroutine MakeGaussMatTyp2(this, mu_dep, dep_N2, dep_N3, mat_num, PntPer)
        ...
        integer                                     :: alpha, beta, gama, i
                                            
        real(dp), dimension(3**ordfc3)              :: coef1, coef
        ...
        alpha_loop: do alpha = 1, 3
            ...
            beta_loop: do beta = 1, 3
                ...
                gama_loop: do gama = 1, 3
                    ...
                    coef1 = this%F(mu_dep)%FCmu(6:, gama, beta, alpha, dep_N3, dep_N2)

                    coef = outkron27_fnl - coef1

                    i = 9*(alpha-1) + 3*(beta-1) + gama !- 1) + 1

                    GaussMatloc(:, i) = coef

                    deallocate( outkron27_fnl )
    ...
    subroutine PointGroup(this, mu_ind, N2_ind, N3_ind, &
                        & writetype2GMat, InterGMat, PntPer, Cut_Off, sys)
        ...
        pntSym_loop: do i = 1, NumPntIndx
            ...
            out_chk: if ( .not. outside ) then 
                ...
                iden_trans = ( (mu_dep == mu_ind) .and. (dep_N2 == N2_ind) .and. (dep_N3 == N3_ind) .and. &
                ...
                same_map: if ( iden_trans ) then
                    ...
                    call this%Exists(mu_dep, dep_N2, dep_N3, -1, &
                                   & mu_ind, N2_ind, N3_ind, dummy_Comb, indx, & 
                                   & InterGMat, writetype2GMat, PntPer, exist_chk, err_var)
                    err_chk: if ( err_var /= 0 ) then
                        write(*, 76) err_var
                        STOP
                    ...
                    fillupdep_Per: if ( (.not. exist_chk) .and. (.not. InterGMat) ) then
                        ...
                        call this%Fillup_cart_dep(mu_dep, dep_N2, dep_N3, -1, &
                                                & mu_ind, N2_ind, N3_ind, indx, PntPer)
        ...
        alpha_loop: do alpha = 1, 3
            ...
            beta_loop: do beta = 1, 3
                ...
                gama_loop: do gama = 1, 3
                    ...
                    outkron27_fnl(i) = outkron27_fnl(i) - 1.0_dp

                    GaussMatloc(:, i) = outkron27_fnl
    ...
    subroutine Fillup_cart_dep(this, mu_dep, dep_N2, dep_N3, dep_var, &
                                   & mu_ind, N2_ind, N3_ind, mat_num, PntPer)
        ...
        alpha_loop: do alpha = 1, 3
            ...
            beta_loop: do beta = 1, 3
                ...
                gama_loop: do gama = 1, 3
                    ...
                    this%F(mu_dep)%FCmu(1:2, gama, beta, alpha, dep_N3, dep_N2) = dble( dep_var )
                    this%F(mu_dep)%FCmu(3:5, gama, beta, alpha, dep_N3, dep_N2) = dble( ind_Indx )
                    this%F(mu_dep)%FCmu(6:, gama, beta, alpha, dep_N3, dep_N2) = outkron27_fnl
    ...
    subroutine cart_dep(this, mu_ind, ind_N2, ind_N3)
        ...
        integer                                     :: num_fc, alpha, res, beta, gama, &
                                                     & alpha_lin, res_lin, beta_lin, gama_lin
        ...
        real(dp), dimension(3**ordfc3)              :: coef, coef_dep, coef_final
        ...
        fc_cart_loop: do fc = num_fc, 1, -1
            ...
            last_fc: if ( fc == num_fc ) then
                ...
                pivot_chk1: if ( dabs(pivot) > EPS ) then
                    ...
                    chk2 = this%F(mu_ind)%FCmu(1, gama, beta, alpha, ind_N3, ind_N2)

                    write8_1: if ( dabs(chk2-1.0_dp) < EPS ) then

                        this%F(mu_ind)%FCmu(1, gama, beta, alpha, ind_N3, ind_N2) = 8.0_dp
                        this%F(mu_ind)%FCmu(6:, gama, beta, alpha, ind_N3, ind_N2) = 0.0_dp
                    ...
                    not_ind_chk1: if ( dabs(chk2-1.0_dp) > EPS ) then
                        ...
                        ! ~ Overwrite ~ !
                        this%F(mu_ind)%FCmu(3:5, gama, beta, alpha, ind_N3, ind_N2) = dble( ind_Indx )
                    ...
                    any_aftr: if ( any(dabs(after) > EPS) ) then
                        ...
                        all_el_loop: do ind = 1, num_el
                            ...
                            nonzero_el: if ( dabs(after(ind)) > EPS ) then
                                ...
                                alpha_lin = (fc_lin_p / 9) + 1  !**!
                                res_lin = mod( fc_lin_p, 9 )
                                beta_lin = (res_lin / 3) + 1    !**!
                                gama_lin = mod(res_lin, 3) + 1  !**!

                                coeff_lin = -1.0_dp * after(ind) / pivot

                                chk_lin = this%F(mu_ind)%FCmu(1, gama_lin, beta_lin, alpha_lin, ind_N3, ind_N2)
                                ...
                                chk_lin_cond: if ( dabs(chk_lin - 1.0_dp) < EPS ) then
                                    ...
                                    coef_dep = this%F(mu_ind)%FCmu(6:, gama_lin, &
                                                            & beta_lin, alpha_lin, ind_N3, ind_N2)
                        ...
                        this%F(mu_ind)%FCmu(1, gama, beta, alpha, ind_N3, ind_N2) = -5.0_dp
                        ! ~ Overwrite ~ !
                        this%F(mu_ind)%FCmu(3:5, gama, beta, alpha, ind_N3, ind_N2) = dble( ind_Indx )
                        this%F(mu_ind)%FCmu(6:, gama, beta, alpha, ind_N3, ind_N2) = coef
                ...
                !else pivot_chk !( dabs(pivot) < EPS )
                    ...
                    chk = this%F(mu_ind)%FCmu(1, gama, beta, alpha, ind_N3, ind_N2)
                ...
                indx_final = this%F(mu_ind)%FCmu(1, gama, beta, alpha, ind_N3, ind_N2)
                coef_final = this%F(mu_ind)%FCmu(6:, gama, beta, alpha, ind_N3, ind_N2)
    ...
    subroutine Create_Index_ind(this, mu_ind, N2_ind, N3_ind)
        ...
        integer, dimension(2*ordfc3)                :: indindx
        ...
        alpha_loop: do alpha = 1, 3
            beta_loop: do beta = 1, 3
                gama_loop: do gama = 1, 3
                    ...
                    chk_indx1 = this%F(mu_ind)%FCmu(1, gama, beta, alpha, N3_ind, N2_ind)

                    ind_chk: if ( dabs(chk_indx1-1.0_dp) < EPS ) then

                        val = this%IndFCPos(mu_ind)%Pos_mu(gama, beta, alpha, N3_ind, N2_ind)
                        ...
                        FILVAL_chk: if ( val /= FILLVAL ) then
                            ...
                            ! ...........::::::::::::: For flat writing :::::::::::::........... !
                            indindx = (/mu_ind, N2_ind, N3_ind, alpha, beta, gama/)
                            ...
                            chk_Mat_size: if ( this%PosIndCntr <= this%Pos_flat_size ) then
                                ...
                                allocate( tmpPosIndx_flat( (2*ordfc3), PosIndx_flatSz2 ) )
                            ...
                            this%IndFCPos(mu_ind)%Pos_mu(gama, beta, alpha, N3_ind, N2_ind) = this%PosIndCntr
