...
subroutine get_command_line(infile, ChnkSz, ZeroEPS, AdvncOut)

    implicit none

    character(len=128), parameter       :: expected_cmd_line="./a.out -inp [INPUT FILE] -ChnkSz [ChunkSize] &
                                                             &-AdvncOut [T/F] -eps [REAL]"
    integer, parameter                  :: MAX_CMD_ARG=8

    character(len=*), intent(out)       :: infile
    real(dp), intent(out)               :: ChnkSz, ZeroEPS
    logical, intent(out)                :: AdvncOut
    
    ! ==================================== Local Variables ==================================== !

    character(len=64)                   :: Chnk_char, AdvncOut_char, eps_char
    ...
    chk_no_cmd: if ( Narg > 0 .and. Narg <= MAX_CMD_ARG ) then
        ...
        loop_args: do 
            ...
            err_chk: if ( err == 0) then
                ...
                chk_cmd_name: if ( cmd_name == "-inp" ) then
                ...
                else if ( cmd_name == "-ChnkSz" ) then chk_cmd_name
                    ...
                    if ( err == 0 ) then
                        ...
                        Chnk_char = cmd_arg
                        dflt_val(2) = .false.
                        read(Chnk_char, *) ChnkSz
                        write(*, 600) ChnkSz
                        600 FORMAT('Chunk Size of Inter-FC Matrix to be written: ', F7.2, 'MB')
                ...
                else if ( cmd_name == "-AdvncOut" ) then chk_cmd_name

                    call get_command_argument(nn, cmd_arg, cmd_arg_len, err)

                    if ( err == 0 ) then

                        AdvncOut_char = cmd_arg
                        dflt_val(3) = .false.
                        read(AdvncOut_char, *) AdvncOut
                        write(*, 700) ( .not. (AdvncOut) )
                        700 FORMAT('Nonadvancing output is set to:', L7)
                ...
                else if ( cmd_name == "-eps" ) then chk_cmd_name
                    ...
                    if ( err == 0 ) then
                        ...
                        eps_char = cmd_arg
                        dflt_val(4) = .false.
                        read(eps_char, *) ZeroEPS
                        write(*, 800) ZeroEPS
                        800 FORMAT('Zero value cut-off is: ', E11.4)
    ...
    dflt_chk_loop: do i = 1, size(dflt_val)
        ...
        outer_if: if ( dflt_val(i) ) then
            ...
            inner_if: if ( i == 1 ) then
                ...
                ChnkSz = 512.0_dp
                write(*, 650) ChnkSz
                650 FORMAT('Chunk Size is set to default: ', F7.2, 'MB')

            else if ( i == 3 ) then inner_if
                AdvncOut = .false.
                write(*, 750) ( .not. (AdvncOut) )
                750 FORMAT('Nonadvancing output is set to default:', L7)

            else if ( i == 4 ) then inner_if
                ZeroEPS = 1.0E-5_dp
                write(*, 850) ZeroEPS
                850 FORMAT('Zero value cut-off is set to default: ', E11.4)
...
subroutine print_err_msg(err, nn, cmd_line, expected_cmd_line)
    ...
    subroutine ShowWelcomeBanner()
        ...
        40 FORMAT(10X, "Executable: FC3.x (Symmetry reduction of 3rd-order IFCs)")
