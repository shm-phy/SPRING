...
module FCmod
    ...
    use kinds,              only : dp
    use constants,          only : EPS_nzero, ordfc4, &
...
contains
    ...
    subroutine init_FCtyp(this, ChnkSz, AdvncOut, sys, Cut_Off, PntPer)
        ...
        mu_loop1: do mu = 1, Nbasis
            ...
            allocate( this%F(mu)%FCmu(cnst,3,3,3,3,Natm,Natm,Natm) )

            this%F(mu)%FCmu = 0.0_dp

            this%TotalFCNum = this%TotalFCNum + ((Natm**3) * (3**ordfc4))
        ...
        mu_loop2: do mu = 1, Nbasis
            ...
            Natm = Cut_Off%numAtom(mu)
            allocate( this%IndFCPos(mu)%Pos_mu(3,3,3,3,Natm,Natm,Natm) )
        ...
        this%PerInter = [ (i, i=2,24) ]

        allocate( this%PntGrIter( NSymApply ) )
        this%PntGrIter = [ (i, i=2,PntPer%Nsym) ]
        this%PerIter = [ (i, i=2,24) ]

        this%Pos_flat_size = initMatsz2
        this%PosIndCntr = 0
        allocate( this%PosIndx_flat( (2*ordfc4), this%Pos_flat_size ) )
        this%PosIndx_flat = 0

        this%InterMatChunk = 0
        this%InterMatPos = 0
        this%InterMatChnkSize = ChnkSz ! in MB
        this%InterMatFile = 'InterMatChunk_4th_'//trim(sys%prefix)//'.h5'
            ...
            N2_loop: do N2 = 1, Natm
                N3_loop: do N3 = 1, Natm
                    N4_loop: do N4 = 1, Natm
                        ...
                        call this%PrimitiveExists(mu, N2, N3, N4, PrmtvExst)

                        apply_pntper: if (.not. PrmtvExst) then

                            call this%ApplySymm(mu, N2, N3, N4, PntPer, Cut_Off, sys)
    ...
    subroutine PrimitiveExists(this, mu, N2, N3, N4, PrmtvExst)
        ...
        else if ( all(dabs(this%F(mu)%FCmu(2,:,:,:,:,N4,N3,N2)) < EPS) ) then prmtv_chk
            PrmtvExst = .false.

        else if ( all(dabs(this%F(mu)%FCmu(2,:,:,:,:,N4,N3,N2) - 1.0_dp) < EPS) &
           & .or. all(dabs(this%F(mu)%FCmu(2,:,:,:,:,N4,N3,N2) + 1.0_dp) < EPS) &
           & .or. all(dabs(this%F(mu)%FCmu(2,:,:,:,:,N4,N3,N2) + 7.0_dp) < EPS) ) then prmtv_chk
    ...
    subroutine ApplySymm(this, mu_ind, N2_ind, N3_ind, N4_ind, PntPer, Cut_Off, sys)
        ...
        call this%Fillup_cartesian(mu_ind, N2_ind, N3_ind, N4_ind, 1)
        ...
        this%NumInterPer = 0
        !this%PerInter(:) = 0
        call this%Permute(mu_ind, N2_ind, N3_ind, N4_ind, &
                        & writetype2GMat, InterGMat, PntPer, Cut_Off)

        this%NumInterPnt = 0
        !this%PntGrInter(:) = 0
        call this%PointGroup(mu_ind, N2_ind, N3_ind, N4_ind, &
                           & writetype2GMat, InterGMat, PntPer, Cut_Off, sys)

        gaus_el_chk: if ( this%IdenCount /= 0 ) then

            call this%cart_dep(mu_ind, N2_ind, N3_ind, N4_ind)
        ...
        call this%Create_Index_ind(mu_ind, N2_ind, N3_ind, N4_ind)
    ...
    subroutine Fillup_cartesian(this, mu_ind, N2_ind, N3_ind, N4_ind, dep_var)
        ...
        ind_Indx = (/mu_ind, N2_ind, N3_ind, N4_ind/)
            ...
            beta_loop: do beta = 1, 3
                gama_loop: do gama = 1, 3
                    delta_loop: do delta = 1, 3
                        ...
                        this%F(mu_ind)%FCmu(1:2, delta, gama, beta, alpha, N4_ind, N3_ind, N2_ind) = dble( dep_var )
                        this%F(mu_ind)%FCmu(3:6, delta, gama, beta, alpha, N4_ind, N3_ind, N2_ind) = dble( ind_Indx )

                        coef = 0.0_dp
                        i_ind = 27*(alpha-1) + 9*(beta-1) + 3*(gama-1) + delta !- 1) + 1
                        coef(i_ind) = coef(i_ind) + 1.0_dp

                        this%F(mu_ind)%FCmu(7:, delta, gama, beta, alpha, N4_ind, N3_ind, N2_ind) = coef
    ...
    subroutine Permute(this, mu_ind, N2_ind, N3_ind, N4_ind, &
                     & writetype2GMat, InterGMat, PntPer, Cut_Off)
        ...
        integer, dimension(23)                      :: PerIndx
        integer, dimension(ordfc4)                  :: comb
        integer, dimension(4, ordfc4)               :: ind_array

        integer, dimension(4)                       :: dep_N1crt_mu, dep_N2crt_nu, &
                                                     & dep_N3crt_et, dep_N4crt_ro

        integer                                     :: mu_dep, dep_N2, dep_N3, dep_N4
        ...
        ind_array(:, 3) = Cut_Off%atmIndx(mu_ind)%cbIndx(:, N3_ind)
        ind_array(:, 4) = Cut_Off%atmIndx(mu_ind)%cbIndx(:, N4_ind)
        ...
        chk_interGMat: if ( InterGMat ) then
            ...
            NumPerIndx = size( PntPer%Per, 2) - 1 !23
        ...
        perindx_loop: do i = 1, NumPerIndx
            ...
            dep_N3crt_et = ind_array(:, comb(3))
            dep_N4crt_ro = ind_array(:, comb(4))
            ...
            dep_N3crt_et(1:3) = dep_N3crt_et(1:3) - N1cell
            dep_N4crt_ro(1:3) = dep_N4crt_ro(1:3) - N1cell

            dep_N2 = get_atom_num(mu_dep, dep_N2crt_nu, Cut_Off)
            dep_N3 = get_atom_num(mu_dep, dep_N3crt_et, Cut_Off)
            dep_N4 = get_atom_num(mu_dep, dep_N4crt_ro, Cut_Off)

            outside = ( (dep_N2 == 0) .or. (dep_N3 == 0) .or. (dep_N4 == 0) )

            out_chk: if ( .not. outside ) then

                same_map: if ( (mu_dep == mu_ind) .and. (dep_N2 == N2_ind) .and. &
                             & (dep_N3 == N3_ind) .and. (dep_N4 == N4_ind) ) then
                    ...
                    call this%Exists(mu_dep, dep_N2, dep_N3, dep_N4, -7, &
                                   & mu_ind, N2_ind, N3_ind, N4_ind, comb, indx, &
                    ...
                    fillupdep_Per: if ( (.not. exist_chk) .and. (.not. InterGMat) ) then
                        ...
                        call this%Fillup_cart_dep_perm(mu_dep, dep_N2, dep_N3, dep_N4, -7, &
                                                     & mu_ind, N2_ind, N3_ind, N4_ind, comb)
    ...
    subroutine MakeGaussMatTyp3(this, comb)
        ...
        real(dp), dimension(3**ordfc4, 3**ordfc4)   :: GaussMatloc
        real(dp), dimension(3**ordfc4)              :: coef
        ...
        integer, dimension(ordfc4)                  :: ind_cart, dep_cart
        integer                                     :: Matsize2
            ...
            beta_loop: do beta = 1, 3
                gama_loop: do gama = 1, 3
                    delta_loop: do delta = 1, 3
                        ...
                        ind_cart = (/alpha, beta, gama, delta/)

                        atm4_loop: do af = 1, ordfc4
                        ...
                        i_dep = 27*(dep_cart(1) - 1) + 9*(dep_cart(2) -1) + 3*(dep_cart(3) - 1) + dep_cart(4) !-1 + 1
        ...
        gaussMatExist: if ( this%IdenCount == 0) then
            allocate( this%GaussElMat(3**ordfc4, 3**ordfc4) )
            ...
            allocate( GaussMatcopy( 3**ordfc4, Matsize2 ) )
            GaussMatcopy = this%GaussElMat
            deallocate( this%GaussElMat )

            allocate( this%GaussElMat( 3**ordfc4, (Matsize2+3**ordfc4) ) )
    ...
    subroutine Exists(this, mu_dep, dep_N2, dep_N3, dep_N4, dep_var, &
                    & mu_ind, N2_ind, N3_ind, N4_ind, comb, mat_num, &
        ...
        real(dp)                                    :: var
        integer, dimension(ordfc4)                  :: ind_Indx, indx_chk

        ! ====================================== Local Variables ====================================== !

        var = this%F(mu_dep)%FCmu(2, 1, 1, 1, 1, dep_N4, dep_N3, dep_N2)
        indx_chk = int( this%F(mu_dep)%FCmu(3:6, 1, 1, 1, 1, dep_N4, dep_N3, dep_N2) )
                    ...
                    inner_if2: if ( dabs(var-dble(dep_var)) < EPS ) then
                        ...
                        inner_if3: if ( .not. InterGMat ) then
                            ...
                            inner_if4: if (writetype2GMat) then
                                ...
                                call this%MakeGaussMatTyp4(mu_dep, dep_N2, dep_N3, dep_N4, comb)
                    ...
                    iner2_if2: if ( ( dabs(var-dble(dep_var)) < EPS ) .or. &
                        ...
                        iner2_if3: if ( .not. InterGMat ) then
                            ...
                            iner2_if4: if (writetype2GMat) then
                                ...
                                call this%MakeGaussMatTyp2(mu_dep, dep_N2, dep_N3, dep_N4, mat_num, PntPer)
                ...
                iner3_if1: if ( ( dabs(var+1.0_dp) < EPS ) .or. &
                    ...
                    iner3_if2: if ( .not. InterGMat ) then
                        ...
                        iner3_if3: if ( dep_var == -7 ) then
                            ...
                            !*! write(*, *) "this%NumInterPer: ", this%NumInterPer, "mat_num: ", mat_num, "dep_var: ", dep_var, &
                            !*!     & "var:", var
                        ...
                        call this%CreateInterGmat(mu_dep, dep_N2, dep_N3, dep_N4, dep_var, &
    ...
    subroutine Fillup_cart_dep_perm(this, mu_dep, dep_N2, dep_N3, dep_N4, dep_var, &
                                        & mu_ind, N2_ind, N3_ind, N4_ind, comb)
            ...
            beta_loop: do beta = 1, 3
                gama_loop: do gama = 1, 3
                    delta_loop: do delta = 1, 3
                        ...
                        this%F(mu_dep)%FCmu(1:2, dep_cart(4), dep_cart(3), dep_cart(2), dep_cart(1), &
                                               & dep_N4, dep_N3, dep_N2) = dble( dep_var )

                        this%F(mu_dep)%FCmu(3:6, dep_cart(4), dep_cart(3), dep_cart(2), dep_cart(1), &
                                               & dep_N4, dep_N3, dep_N2) = dble( ind_Indx )

                        this%F(mu_dep)%FCmu(7:, dep_cart(4), dep_cart(3), dep_cart(2), dep_cart(1), &
                                              & dep_N4, dep_N3, dep_N2) = coef
    ...
    subroutine MakeGaussMatTyp4(this, mu_dep, dep_N2, dep_N3, dep_N4, comb)
        ...
        real(dp), dimension(3**ordfc4)              :: coef, coef1
            ...
            beta_loop: do beta = 1, 3
                gama_loop: do gama = 1, 3
                    delta_loop: do delta = 1, 3
                        ...
                        coef1 = this%F(mu_dep)%FCmu(7:, dep_cart(4), dep_cart(3), dep_cart(2), dep_cart(1), &
                                                      & dep_N4, dep_N3, dep_N2)

                        coef = (coef - coef1)

                        i = 27*(alpha-1) + 9*(beta-1) + 3*(gama-1) + delta !- 1) + 1
    ...
    subroutine MakeGaussMatTyp2(this, mu_dep, dep_N2, dep_N3, dep_N4, mat_num, PntPer)
        ...
        real(dp), dimension(3**ordfc4)              :: coef1, coef
        ...
        real(dp), dimension(:, :), allocatable      :: GaussMatcopy
        integer                                     :: alpha, beta, gama, delta, i
            ...
            beta_loop: do beta = 1, 3
                ...
                gama_loop: do gama = 1, 3
                    ...
                    delta_loop: do delta = 1, 3
                        ...
                        coef1 = this%F(mu_dep)%FCmu(7:, delta, gama, beta, alpha, dep_N4, dep_N3, dep_N2)

                        coef = outkron81_fnl - coef1

                        i = 27*(alpha-1) + 9*(beta-1) + 3*(gama-1) + delta !- 1) + 1

                        GaussMatloc(:, i) = coef

                        deallocate( outkron9_2 )
                        deallocate( outkron81_fnl )
    ...
    subroutine PointGroup(this, mu_ind, N2_ind, N3_ind, N4_ind, &
                        & writetype2GMat, InterGMat, PntPer, Cut_Off, sys)
        ...
        ind_N3crt_et(:) = Cut_Off%atmIndx(mu_ind)%cbIndx(:, N3_ind)
        ind_N4crt_ro(:) = Cut_Off%atmIndx(mu_ind)%cbIndx(:, N4_ind)
        ...
        pntSym_loop: do i = 1, NumPntIndx
            ...
            dep_N3crt_et = ind_N3crt_et
            dep_N4crt_ro = ind_N4crt_ro
            
            call TransformAtoms(sys, dep_N1crt_mu, dep_N2crt_nu, &
                                   & dep_N3crt_et, dep_N4crt_ro, Rot, Trans)
            ...
            out_chk: if ( .not. outside ) then 
                ...
                iden_trans = ( (mu_dep == mu_ind) .and. (dep_N2 == N2_ind) .and. &
                             & (dep_N3 == N3_ind) .and. (dep_N4 == N4_ind) .and. &
                ...
                same_map: if ( iden_trans ) then
                    ...
                    call this%Exists(mu_dep, dep_N2, dep_N3, dep_N4, -1, &
                                   & mu_ind, N2_ind, N3_ind, N4_ind, dummy_Comb, indx, & 
                                   & InterGMat, writetype2GMat, PntPer, exist_chk, err_var)
                    err_chk: if ( err_var /= 0 ) then
                        write(*, 76) err_var
                        STOP
                    ...
                    fillupdep_Per: if ( (.not. exist_chk) .and. (.not. InterGMat) ) then
                        ...
                        call this%Fillup_cart_dep(mu_dep, dep_N2, dep_N3, dep_N4, -1, &
                                                & mu_ind, N2_ind, N3_ind, N4_ind, indx, PntPer)
            ...
            beta_loop: do beta = 1, 3
                ...
                gama_loop: do gama = 1, 3
                    ...
                    delta_loop: do delta = 1, 3
                        ...
                        outkron81_fnl(i) = outkron81_fnl(i) - 1.0_dp

                        GaussMatloc(:, i) = outkron81_fnl
    ...
    subroutine Fillup_cart_dep(this, mu_dep, dep_N2, dep_N3, dep_N4, dep_var, &
                                   & mu_ind, N2_ind, N3_ind, N4_ind, mat_num, PntPer)
            ...
            beta_loop: do beta = 1, 3
                ...
                gama_loop: do gama = 1, 3
                    ...
                    delta_loop: do delta = 1, 3
                        ...
                        this%F(mu_dep)%FCmu(1:2, delta, gama, beta, alpha, dep_N4, dep_N3, dep_N2) = dble( dep_var )
                        this%F(mu_dep)%FCmu(3:6, delta, gama, beta, alpha, dep_N4, dep_N3, dep_N2) = dble( ind_Indx )
                        this%F(mu_dep)%FCmu(7:, delta, gama, beta, alpha, dep_N4, dep_N3, dep_N2) = outkron81_fnl
    ...
    subroutine cart_dep(this, mu_ind, ind_N2, ind_N3, ind_N4)
        ...
        real(dp), dimension(3**ordfc4)              :: coef, coef_dep, coef_final

        integer                                     :: num_fc, alpha, res1, beta, res2, gama, delta, &
                                                     & alpha_lin, res1_lin, beta_lin, res2_lin, &
                                                     & gama_lin, delta_lin

        integer, dimension(ordfc4)                  :: ind_Indx

        integer                                     :: fc, fc_p, num_el, ind, fc_lin, fc_lin_p, &
                                                     & p
        !*! debug !*!
        !*! integer                                     :: di
        !*! debug !*!
        ...
        num_fc = 3**ordfc4
        ind_Indx = (/mu_ind, ind_N2, ind_N3, ind_N4/)
        ...
        !*! debug !*!
        !*! write(*, *)
        !*! do di = 1, size(GaussElMatin_T, 1)
        !*!     write(*, 75) GaussElMatin_T(di, :)
        !*! end do
        !*! write(*, *)
        !*! debug !*!
        ...
        !*! debug !*!
        !*! write(*, *)
        !*! do di = 1, size(GausMatT, 1)
        !*!     write(*, 75) GausMatT(di, :)
        !*! end do
        !*! write(*, *)

        !*! 75 FORMAT("[", 80(F7.4, ', '), F7.4, "]")
        !*! debug !*!
        ...
        fc_cart_loop: do fc = num_fc, 1, -1
            ...
            last_fc: if ( fc == num_fc ) then
                ...
                pivot_chk1: if ( dabs(pivot) > EPS ) then
                    ...
                    chk2 = this%F(mu_ind)%FCmu(1, delta, gama, beta, alpha, ind_N4, ind_N3, ind_N2)

                    write8_1: if ( dabs(chk2-1.0_dp) < EPS ) then

                        this%F(mu_ind)%FCmu(1, delta, gama, beta, alpha, ind_N4, ind_N3, ind_N2) = 8.0_dp
                        this%F(mu_ind)%FCmu(7:, delta, gama, beta, alpha, ind_N4, ind_N3, ind_N2) = 0.0_dp
                    ...
                    not_ind_chk1: if ( dabs(chk2-1.0_dp) > EPS ) then
                        ...
                        ! ~ Overwrite ~ !
                        this%F(mu_ind)%FCmu(3:6, delta, gama, beta, alpha, ind_N4, ind_N3, ind_N2) = dble( ind_Indx )
                    ...
                    any_aftr: if ( any(dabs(after) > EPS) ) then
                        ...
                        all_el_loop: do ind = 1, num_el
                            ...
                            nonzero_el: if ( dabs(after(ind)) > EPS ) then
                                ...
                                alpha_lin = (fc_lin_p / 27) + 1     !**!
                                res1_lin = mod( fc_lin_p, 27 )
                                beta_lin = (res1_lin / 9) + 1       !**!
                                res2_lin = mod( res1_lin, 9 )
                                gama_lin = (res2_lin / 3) + 1       !**!
                                delta_lin = mod( res2_lin, 3 ) + 1  !**!

                                coeff_lin = -1.0_dp * after(ind) / pivot

                                chk_lin = this%F(mu_ind)%FCmu(1, delta_lin, gama_lin, &
                                                            & beta_lin, alpha_lin, ind_N4, ind_N3, ind_N2)
                                ...
                                chk_lin_cond: if ( dabs(chk_lin - 1.0_dp) < EPS ) then
                                    ...
                                    coef_dep = this%F(mu_ind)%FCmu(7:, delta_lin, gama_lin, &
                                                            & beta_lin, alpha_lin, ind_N4, ind_N3, ind_N2)
                        ...
                        this%F(mu_ind)%FCmu(1, delta, gama, beta, alpha, ind_N4, ind_N3, ind_N2) = -5.0_dp
                        ! ~ Overwrite ~ !
                        this%F(mu_ind)%FCmu(3:6, delta, gama, beta, alpha, ind_N4, ind_N3, ind_N2) = dble( ind_Indx )
                        this%F(mu_ind)%FCmu(7:, delta, gama, beta, alpha, ind_N4, ind_N3, ind_N2) = coef
                ...
                !else pivot_chk !( dabs(pivot) < EPS )
                    ...
                    chk = this%F(mu_ind)%FCmu(1, delta, gama, beta, alpha, ind_N4, ind_N3, ind_N2)
                ...
                indx_final = this%F(mu_ind)%FCmu(1, delta, gama, beta, alpha, ind_N4, ind_N3, ind_N2)
                coef_final = this%F(mu_ind)%FCmu(7:, delta, gama, beta, alpha, ind_N4, ind_N3, ind_N2)
    ...
    subroutine Create_Index_ind(this, mu_ind, N2_ind, N3_ind, N4_ind)
        ...
        integer, dimension(2*ordfc4)                :: indindx
            ...
            beta_loop: do beta = 1, 3
                gama_loop: do gama = 1, 3
                    delta_loop: do delta = 1, 3
                        ...
                        chk_indx1 = this%F(mu_ind)%FCmu(1, delta, gama, beta, alpha, N4_ind, N3_ind, N2_ind)

                        ind_chk: if ( dabs(chk_indx1-1.0_dp) < EPS ) then

                            val = this%IndFCPos(mu_ind)%Pos_mu(delta, gama, beta, alpha, N4_ind, N3_ind, N2_ind)
                            ...
                            FILVAL_chk: if ( val /= FILLVAL ) then
                                ...
                                ! ...........::::::::::::: For flat writing :::::::::::::........... !
                                indindx = (/mu_ind, N2_ind, N3_ind, N4_ind, alpha, beta, gama, delta/)
                                ...
                                chk_Mat_size: if ( this%PosIndCntr <= this%Pos_flat_size ) then
                                    ...
                                    allocate( tmpPosIndx_flat( (2*ordfc4), PosIndx_flatSz2 ) )
                                ...
                                this%IndFCPos(mu_ind)%Pos_mu(delta, gama, beta, alpha, &
                                                           & N4_ind, N3_ind, N2_ind) = this%PosIndCntr
...
#ifdef OMP
    include "Reduce_separate_OMP.f90"
    include "ASR_separate_OMP.f90"
#else
