...
subroutine ReducePntPerASR(this, Nbasis, ZeroEPS, Cut_Off)
    ...
    integer                                 :: numfc
    integer, dimension(2*ordfc4)            :: FCindx
    ...
    mu_loop: do mu = 1, Nbasis
        ...
        allocate( this%PosIndx_fin(mu)%Pos_mu(3,3,3,3,Natm,Natm,Natm), STAT=istat, ERRMSG=msg )
        if ( istat /= 0 ) call this%AllocationError( istat, msg )
    ...
    allocate( this%PosIndx_flat_fin( (2*ordfc4), numfc ) )
    ...
    fc_loop: do nf = numfc, 1, -1
        ...
        !(/mu_ind, N2_ind, N3_ind, N4_ind, alpha, beta, gama, delta/)
        ...
        PivotChk: if ( dabs(pivot) > ZeroEPS ) then
            ...
            AftrChk: if ( (size(after) == 0) .or. (all(dabs(after) < ZeroEPS)) ) then
                ...
                this%F(FCindx(1))%FCmu(1, FCindx(8), FCindx(7), FCindx(6), FCindx(5), &
    ...
    ResizePosIndxFlat: if ( this%PosIndCntr_fin == this%Indfin ) then
        ...
        allocate( this%PosIndx_flat_fin((2*ordfc4), NumCol) )
    ...
    80 FORMAT("The FC corresponding to the indx: [", 7(I3, ', '), I3, "] is zero.")
...
subroutine MakeIndIndxFin(this, FCindx)
    ...
    integer, dimension(2*ordfc4), intent(in)    :: FCindx
    ...
    val = this%PosIndx_fin(FCindx(1))%Pos_mu(FCindx(8), FCindx(7), FCindx(6), FCindx(5), &
    ...
    ValChk: if ( val /= FILLVAL ) then
        ...
        this%PosIndx_fin(FCindx(1))%Pos_mu(FCindx(8), FCindx(7), FCindx(6), FCindx(5), &
