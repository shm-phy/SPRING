...
subroutine get_command_line(infile, T, freq_cut, qMeshL, AnharFreeEng, HarmonicLarge)

    implicit none

    character(len=512), parameter       :: expected_cmd_line="./a.out -inp [INPUT FILE] -T [TEMPERATURE] &
                                                            & -omegacut [Omega Cut-Off] -AnHar [T/F] -LargeQHar [T/F] &
                                                            & -QLarge [Integer]"
    integer, parameter                  :: MAX_CMD_ARG=12

    character(len=*), intent(out)       :: infile
    real(dp), intent(out)               :: T, freq_cut
    integer, intent(out)                :: qMeshL
    logical, intent(out)                :: AnharFreeEng, HarmonicLarge
    ...
    chk_no_cmd: if ( Narg > 0 .and. Narg <= MAX_CMD_ARG ) then
        ...
        loop_args: do 
            ...
            err_chk: if ( err == 0) then
                ...
                chk_cmd_name: if ( cmd_name == "-inp" ) then
                ...
                else if ( cmd_name == "-AnHar" ) then chk_cmd_name
                    ...
                    if ( err == 0 ) then
                        ...
                        Temp_char = cmd_arg
                        dflt_val(4) = .false.
                        read(Temp_char, *) AnharFreeEng

                        if ( this_image() == 1) then
                            write(*, 701) AnharFreeEng
                            701 FORMAT('Anharmonic Free Energy calculation is set to: ', L7)
                        end if 
                ...
                else if ( cmd_name == "-LargeQHar" ) then chk_cmd_name
                    ...
                    if ( err == 0 ) then
                        ...
                        Temp_char = cmd_arg
                        dflt_val(5) = .false.
                        read(Temp_char, *) HarmonicLarge

                        if ( this_image() == 1) then
                            write(*, 903) HarmonicLarge
                            903 FORMAT('Harmonic Free Energy for a larger mesh grid is set to: ', L7)
                        end if 
                ...
                else if ( cmd_name == "-QLarge" ) then chk_cmd_name
                    ...
                    if ( err == 0 ) then
                        ...
                        Temp_char = cmd_arg
                        dflt_val(6) = .false.
                        read(Temp_char, *) qMeshL

                        if ( this_image() == 1) then
                            write(*, 702) qMeshL, qMeshL, qMeshL
                            702 FORMAT('Larger Q-mesh for Harmonic Free-Energy is set to: ( ', 2(I3, ' X '), I3, ' )')
                        end if 
    ...
    dflt_chk_loop: do i = 1, size(dflt_val)
        ...
        outer_if: if ( dflt_val(i) ) then
            ...
            inner_if: if ( i == 1 ) then
                ...
                AnharFreeEng = .true.
                if ( this_image() == 1) then
                    write(*, 751) AnharFreeEng
                    751 FORMAT('Anharmonic Free Energy calculation is set to default: ', L7)
                end if 
                ...
                !-! if ( this_image() == 1) then
                !-!     write(*, 917) HarmonicLarge
                !-!     917 FORMAT('Harmonic Free Energy for a larger mesh group is set to default: ', L7)
                !-! end if 

            else if ( i == 6 ) then inner_if
                qMeshL = 17

                !-! if ( (this_image() == 1) .and. HarmonicLarge ) then
                !-!     write(*, 921) qMeshL, qMeshL, qMeshL
                !-!     921 FORMAT('Larger Q-mesh for Harmonic Free-Energy is set to default: ( ', 2(I3, ' X '), I3, ' )')
                !-! end if 
...
subroutine print_err_msg(err, nn, cmd_line, expected_cmd_line)
    ...
    subroutine ShowWelcomeBanner()
        ...
        40 FORMAT(10X, "Executable: FreeEng.x (Free Energy calculation: harmonic and anharmonic)")
