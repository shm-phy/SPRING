...
module DynaMat
    ...
    public  :: get_phonon, get_phonon_pd, get_phonon_singleq
...
subroutine get_phonon_pd( sys, FC2, EwaldConst, Nq, Ndof, myQsize, myOffset, &
    ...
    complex(dp), dimension(Ndof, Ndof, Ndof, Nq), intent(inout) :: ProjOp
    complex(dp), dimension(Ndof, Ndof, Nq), intent(inout)       :: Evec
    real(dp), dimension(Ndof, Nq), intent(inout)                :: freq
    real(dp), intent(out)                                       :: freq_min, freq_max

    !! ====================================== Local variables ====================================== !!

    complex(dp), dimension(:,:), allocatable                :: Dyn
    complex(dp), dimension(:,:), allocatable                :: DynEw, eg_col, eg_row, ProjOp_q
    complex(dp), dimension(:), allocatable                  :: eg_vec

    real(dp), dimension(3)                                  :: q, qhat
    real(dp), dimension(:), allocatable                     :: W

    integer                                                 :: ii, jj, my_i, df
    ...
    allocate( eg_col(Ndof, 1) )
    eg_col = dcmplx(0.0_dp, 0.0_dp)

    allocate( eg_row(1, Ndof) )
    eg_row = dcmplx(0.0_dp, 0.0_dp)

    allocate( ProjOp_q(Ndof, Ndof) )
    ProjOp_q = dcmplx(0.0_dp, 0.0_dp)
    ...
    q_loop: do my_i = 1, myQsize
        ...
        do jj = 1, size(neg_check)
            ...
            warning: if ( neg_check(jj) ) then 
                ...
                write(*, 54) q, (W(jj) * wTHz)
                54 FORMAT("WARNING: Imaginary frequency at q = (", &
                         & 2(F9.4, ', '), F9.4, "). Freqency (omega) = ", F9.4, "rad/ps")
            end if warning
        ...
        df_loop: do df = 1, Ndof

            eg_vec = Dyn(:, df)

            eg_col(:, 1) = eg_vec
            eg_row(1, :) = DCONJG( eg_vec )
            ProjOp_q = matmul( eg_col, eg_row )

            ProjOp(:, :, df, ii) = ProjOp_q
    ...
    deallocate( neg_check )
    deallocate( eg_vec, eg_col, eg_row, ProjOp_q )

end subroutine get_phonon_pd
