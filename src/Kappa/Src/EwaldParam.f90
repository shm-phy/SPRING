...
module EwaldMod
    ...
    contains
        procedure, public, pass         :: set_EwaldParam
...
contains
    ...
    subroutine set_EwaldParam(this, sys, filename)
        ...
        open(unit=5, file=filename, status='OLD', iostat=err, iomsg=err_msg, &
            ...
            read(unit=5, nml=EwaldInfo, iostat=err, iomsg=err_msg)
        ...
        call mesh_points(sys%latvec, this%Rmesh, .false., .true., Rpoints)
        call mesh_points(sys%G, this%Gmesh, .false., .true., Gpoints)
