...
contains
    ...
    !** Debug Purpose **!
    Function FindIverxFver( Iver, F_ver_srt, enterCase ) Result( IxF )
        ...
        ! ** For debug purpose only. Same as => dot_product( Iver, F_ver_srt ) * OnebySix 
        chk_entercase: if ( enterCase ) then
    ...
    end Function FindIverxFver
    !** Debug Purpose **!

    subroutine FindFandOmegaAtVertices( Ndof, Nq, s0, s1, s2, &
                                      & q1IndxVer_unsrt, q2IndxVer_unsrt, &
                                      & ph_q1, W_s0s1s2, &
        ...
        vert_loop: do vv = 1, 4
            ...
            F_ver1(vv) = W_s0s1s2( s2, s1, s0, q1IndxVer_unsrt(vv) ) * &
                       & ( ph_q1%nBE(s1, q1IndxVer_unsrt(vv)) + ph_q1%nBE(s2, q2IndxVer_unsrt(vv)) + 1.0_dp )

            F_ver2(vv) = W_s0s1s2( s2, s1, s0, q1IndxVer_unsrt(vv) ) * &
                       & ( ph_q1%nBE(s1, q1IndxVer_unsrt(vv)) - ph_q1%nBE(s2, q2IndxVer_unsrt(vv)) )

            M_ver1(vv) = ph_q1%omega(s1, q1IndxVer_unsrt(vv)) + ph_q1%omega(s2, q2IndxVer_unsrt(vv))
            M_ver2(vv) = ph_q1%omega(s2, q2IndxVer_unsrt(vv)) - ph_q1%omega(s1, q1IndxVer_unsrt(vv))
            M_ver3(vv) = ph_q1%omega(s1, q1IndxVer_unsrt(vv)) - ph_q1%omega(s2, q2IndxVer_unsrt(vv))
    ...
    subroutine FindFandOmegaAtVertices_Iso( Ndof, Nq, s0, s1, q1IndxVer_unsrt, &
                                          & ph_q1, W_s0s1, F_ver, M_ver)

        implicit none

        integer, intent(in)                                     :: Ndof, Nq, s0, s1
        integer, dimension(4), intent(in)                       :: q1IndxVer_unsrt
        type(Phon), intent(in)                                  :: ph_q1
        real(dp), dimension(Ndof,Ndof,Nq), intent(in)           :: W_s0s1

        real(dp), dimension(4), intent(out)                     :: F_ver, M_ver
        ...
        vert_loop: do vv = 1, 4
            ...
            F_ver(vv) = W_s0s1( s1, s0, q1IndxVer_unsrt(vv) ) 

            M_ver(vv) = ph_q1%omega(s1, q1IndxVer_unsrt(vv)) 

        end do vert_loop

    end subroutine FindFandOmegaAtVertices_Iso
