...
subroutine get_command_line(infile, T, freq_cut, SpectralResolve, ModeResolve, Symm)

    implicit none

    character(len=512), parameter       :: expected_cmd_line="./a.out -inp [INPUT FILE] -T [TEMPERATURE] &
                                                            & -omegacut [Omega Cut-Off] -Spectral_k [T/F] -Mode_resolved_k [T/F] &
                                                            & -Symm [T/F]"
    integer, parameter                  :: MAX_CMD_ARG=12

    character(len=*), intent(out)       :: infile
    real(dp), intent(out)               :: T, freq_cut
    logical, intent(out)                :: SpectralResolve, ModeResolve, Symm
    ...
    chk_no_cmd: if ( Narg > 0 .and. Narg <= MAX_CMD_ARG ) then
        ...
        loop_args: do 
            ...
            err_chk: if ( err == 0) then
                ...
                chk_cmd_name: if ( cmd_name == "-inp" ) then
                ...
                else if ( cmd_name == "-Spectral_k" ) then chk_cmd_name
                    ...
                    if ( err == 0 ) then
                        ...
                        Temp_char = cmd_arg
                        dflt_val(4) = .false.
                        read(Temp_char, *) SpectralResolve

                        img1_chk7: if ( this_image() == 1 ) then
                            write(*, 455) SpectralResolve
                            455 FORMAT('Spectral Resolved thermal-conductivity:', L7)
                        end if img1_chk7
                ...
                else if ( cmd_name == "-Mode_resolved_k" ) then chk_cmd_name
                    ...
                    if ( err == 0 ) then
                        ...
                        Temp_char = cmd_arg
                        dflt_val(5) = .false.
                        read(Temp_char, *) ModeResolve

                        img1_chk8: if ( this_image() == 1 ) then
                            write(*, 555) ModeResolve
                            555 FORMAT('Mode Resolved thermal-conductivity:', L7)
                        end if img1_chk8
                ...
                else if ( cmd_name == "-Symm" ) then chk_cmd_name
                    ...
                    if ( err == 0 ) then
                        ...
                        img1_chk_9: if ( this_image() == 1 ) then
                            write(*, 535) Symm
                            535 FORMAT('Symmetrization of IBZ phonon group velocity: ', L7)
                        end if img1_chk_9
    ...
    dflt_chk_loop: do i = 1, size(dflt_val)
        ...
        outer_if: if ( dflt_val(i) ) then
            ...
            inner_if: if ( i == 1 ) then
                ...
                SpectralResolve = .false.
                !-! img1_chk9: if ( this_image() == 1) then
                !-!     write(*, 755) SpectralResolve
                !-!     755 FORMAT('Spectral resolve is set to default: ', L7)
                !-! end if img1_chk9

            else if ( i == 5 ) then inner_if
                ModeResolve = .false.
                !-! img1_chk10: if ( this_image() == 1) then
                !-!     write(*, 855) ModeResolve
                !-!     855 FORMAT('Mode resolve is set to default: ', L7)
                !-! end if img1_chk10

            else if ( i == 6 ) then inner_if
                Symm = .true.
                !-! img1_chk11: if ( this_image() == 1) then
                !-!     write(*, 835) Symm
                !-!     835 FORMAT('Symmetrization of IBZ phonon group velocity set to default: ', L7)
                !-! end if img1_chk11
...
subroutine print_err_msg(err, nn, cmd_line, expected_cmd_line)
    ...
    subroutine ShowWelcomeBanner()
...
#endif
        write(*, 15) today
        15 FORMAT( 10X, "Program starts on: ", A32 )
        write(*, 50)
        write(*, 10)
        ...
        40 FORMAT(10X, "Executable: kappa.x (Thermal conductivity from 3-phonon and 4-phonon process)")
