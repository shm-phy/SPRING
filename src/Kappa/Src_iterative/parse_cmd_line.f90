...
subroutine get_command_line(infile, T, Tempk)

    implicit none

    character(len=512), parameter       :: expected_cmd_line="./a.out -inp [INPUT FILE] -T [TEMPERATURE] -Tempk [TEMPERATURE]"
    integer, parameter                  :: MAX_CMD_ARG=6

    character(len=*), intent(out)       :: infile
    real(dp), intent(out)               :: T, Tempk
    ...
    chk_no_cmd: if ( Narg > 0 .and. Narg <= MAX_CMD_ARG ) then
        ...
        loop_args: do 
            ...
            err_chk: if ( err == 0) then
                ...
                chk_cmd_name: if ( cmd_name == "-inp" ) then
                ...
                else if ( cmd_name == "-Tempk" ) then chk_cmd_name
                    ...
                    if ( err == 0 ) then
                        Temp_char = cmd_arg
                        dflt_val(3) = .false.
                        read(Temp_char, *) Tempk

                        img1_chk5: if ( this_image() == 1) then
                            write(*, 700) Tempk
                            700 FORMAT('Temperature for kappa calculation: ', F7.2, 'K')
                        end if img1_chk5
        ...
        outer_if: if ( dflt_val(i) ) then
            ...
            inner_if: if ( i == 1 ) then
                ...
                img1_chk6: if ( this_image() == 1) then
                    write(*, 750) Tempk
                    750 FORMAT('Temperature for kappa calculation is set to default: ', F7.2, 'K')
                end if img1_chk6
...
subroutine print_err_msg(err, nn, cmd_line, expected_cmd_line)
    ...
    subroutine ShowWelcomeBanner()
        ...
        40 FORMAT(10X, "Executable: kappa_iter.x (Iterative solution of BTE)")
