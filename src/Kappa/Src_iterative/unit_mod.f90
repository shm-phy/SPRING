...
module unit_cell
    ...
    type, public :: cell
        ...
        real(dp), dimension(:), allocatable     :: OnebySqrtMass
...
contains
    ...
    subroutine init_data(this, filename)
        ...
        open(unit=5, file=filename, status='OLD', iostat=err, iomsg=err_msg, &
           & action='READ', delim='APOSTROPHE')
            ...
            read(unit=5, nml=FCInfo, iostat=err, iomsg=err_msg)
            ReadChk1: if ( err /= 0 ) then
                write(*, 134) filename
                write(*, *) 'Error message : ', err_msg
            end if ReadChk1

            read(unit=5, nml=CrystalInfo, iostat=err, iomsg=err_msg)
            ReadChk2: if ( err /= 0 ) then
                write(*, 134) filename
                write(*, *) 'Error message : ', err_msg
            end if ReadChk2
        ...
        open(unit=5, file=filename, status='OLD', iostat=err, iomsg=err_msg, &
           & action='READ', delim='APOSTROPHE')
            ...
            read(unit=5, nml=AtmInfo, iostat=err, iomsg=err_msg)
            ReadChk3: if ( err /= 0 ) then
                write(*, 134) filename
                write(*, *) 'Error message : ', err_msg
            end if ReadChk3
        ...
        allocate( this%OnebySqrtMass(this%natm) ) !**_**!

        do i = 1, this%natm
            atm_typ_indx = this%typ_indx(i)
            this%mass(i) = this%mass_unq(atm_typ_indx)
            this%OnebySqrtMass(i) = 1.0_dp / dsqrt( this%mass(i) ) !**_**!
        end do
    ...
    function cross(a, b)
        ...
        real(dp), dimension(3) :: cross !Result
