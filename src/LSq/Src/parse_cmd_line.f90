...
subroutine get_command_line(infile, T, ZeroEPS, Renorm, PointDef)

    implicit none

    character(len=128), parameter       :: expected_cmd_line="./a.out -inp [INPUT FILE] -T [TEMPERATURE] &
                                                             &-Renorm [T/F] -PointDef [T/F] -CutOff [REAL]"
    integer, parameter                  :: MAX_CMD_ARG=10

    character(len=*), intent(out)       :: infile
    real(dp), intent(out)               :: T, ZeroEPS
    logical, intent(out)                :: Renorm, PointDef
    
    ! ==================================== Local Variables ==================================== !

    character(len=64)                   :: T_char, Renorm_char, eps_char
    ...
    chk_no_cmd: if ( Narg > 0 .and. Narg <= MAX_CMD_ARG ) then
        ...
        loop_args: do 
            ...
            err_chk: if ( err == 0) then
                ...
                chk_cmd_name: if ( cmd_name == "-inp" ) then
                ...
                else if ( cmd_name == "-Renorm" ) then chk_cmd_name
                    ...
                    if ( err == 0 ) then
                        ...
                        Renorm_char = cmd_arg
                        dflt_val(3) = .false.
                        read(Renorm_char, *) Renorm
                        write(*, 700) Renorm
                        700 FORMAT('Renormalization flag is set to:', L7)
                ...
                else if ( cmd_name == "-PointDef" ) then chk_cmd_name
                    ...
                    if ( err == 0 ) then
                        ...
                        Renorm_char = cmd_arg
                        dflt_val(4) = .false.
                        read(Renorm_char, *) PointDef
                        write(*, 701) PointDef
                        701 FORMAT('Point-defect flag is set to:', L7)
                ...
                else if ( cmd_name == "-CutOff" ) then chk_cmd_name
                    ...
                    if ( err == 0 ) then
                        ...
                        write(*, 800) ZeroEPS
                        800 FORMAT('Singular value cut-off is: ', E11.4)
    ...
    dflt_chk_loop: do i = 1, size(dflt_val)
        ...
        outer_if: if ( dflt_val(i) ) then
            ...
            inner_if: if ( i == 1 ) then
                ...
                Renorm = .false.
                write(*, 750) Renorm
                750 FORMAT('Renorm flag is set to default:', L7)

            else if ( i == 4 ) then inner_if
                PointDef = .false.
                write(*, 751) PointDef
                751 FORMAT('PointDef flag is set to default:', L7)

            else if ( i == 5 ) then inner_if
                ZeroEPS = 1.0E-5_dp
                write(*, 850) ZeroEPS
                850 FORMAT('Singular value cut-off is set to default: ', E11.4)
...
subroutine print_err_msg(err, nn, cmd_line, expected_cmd_line)
    ...
    subroutine ShowWelcomeBanner()
        ...
        40 FORMAT(10X, "Executable: LSq.x (Find IFCs from Least Square of displaemant matrix)")
