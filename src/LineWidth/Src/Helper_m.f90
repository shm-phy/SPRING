...
contains
    ...
    subroutine DistributeQpoints(Nqs, my_Qsize, my_offset, my_edge)
        ...
        SYNC ALL
        call co_broadcast( QinEachProcs, 1 )
        SYNC ALL
    ...
    subroutine WriteAsciiOutFile(filename, write_frmt, dist, freq, LW_q0, Ndof, HeaderWrite)
        ...
        FirstWrite: if ( HeaderWrite ) Then
            ...
            open( unit=ui, file=filename, status='REPLACE', action='WRITE', &
                ...
                open_chk1: if ( err /= 0 ) then
                    ...
                    write(unit=ui, fmt=write_frmt) dist, freq, LW_q0

                    HeaderWrite = .false.

                    20 FORMAT( '#        distance        ', &
                             & '--------- phonon_frequency(rad.THz) ( ', I4, ' columns) ---------      ', &
