...
module FC2_data_mod
    ...
    type, public    :: FC2_dat
        ...
        real(dp), dimension(:), allocatable                         :: FC2_ind

    contains
        procedure, public, pass         :: set_FC2dat

    end type FC2_dat

contains

subroutine set_FC2dat(this, filename, fc2indfc_file)
    ...
    write(*, 132) filename, fc2indfc_file
    132 FORMAT('Reading Second order FC data from file: ', A24, ', and independent FC file: ', A24)

    !.............................................. FC2_ind ................................................!
    call h5open_f(hdferr)

    call h5fopen_f(fc2indfc_file, H5F_ACC_RDONLY_F, file_id, hdferr) !output -> file_id
        call h5gopen_f(file_id, grp_name_indfc, grp_id1, hdferr) !Open and existing group, output -> grp_id1
            ...
            call h5sget_simple_extent_dims_f(space_id, dims1, maxdims1, hdferr) !output -> dims1
                                                                                ! maxdims1
            allocate( this%FC2_ind(1:dims1(1)) )
            call h5dread_f(dset_id, type_id, this%FC2_ind, dims1, hdferr) !Read in array 'FC2_ind'
    ...
    call h5fopen_f(filename, H5F_ACC_RDONLY_F, file_id, hdferr) !output -> file_id
        call h5gopen_f(file_id, grp_name1, grp_id1, hdferr) !Open and existing group, output -> grp_id1
            ...
            !...................................... PosIndx ..........................................!
            allocate( this%PosIndx(3, 3, Natm, Nbasis) )
            this%PosIndx = 0
            ...
            call h5gopen_f(grp_id1, grp_name2, grp_id_in, hdferr) !Open and existing group, output -> grp_id_in
                ...
                mu_loop1: do mu = 1, Nbasis
                    ...
                    call h5sget_simple_extent_dims_f(space_id, dims3, maxdims3, hdferr) !output -> dims3
                                                                                        ! maxdims3
                    allocate( PosIndx_part(1:dims3(1), 1:dims3(2), 1:dims3(3)) )

                    call h5dread_f(dset_id, type_id, PosIndx_part, dims3, hdferr) !Read in array 'PosIndx_part'
            ...
            !.......................................... F ............................................!
            allocate( this%F(LAST_DIM, 3, 3, Natm, Nbasis) )
            this%F = 0.0_dp
            ...
            call h5gopen_f(grp_id1, grp_name3, grp_id_in, hdferr) !Open and existing group, output -> grp_id_in
                ...
                mu_loop2: do mu = 1, Nbasis
                    ...
                    write(mu_char, frmt) mu
                    dset_name_var = 'FC2_mu'//trim(adjustl(adjustr(mu_char)))

                    call h5dopen_f(grp_id_in, dset_name_var, dset_id, hdferr) !Opens an existing dataset.
                                                                              !output -> dset_id
                    call h5dget_type_f(dset_id, type_id, hdferr) !output -> type_id
                    call h5dget_space_f(dset_id, space_id, hdferr) !output -> space_id
                    call h5sget_simple_extent_dims_f(space_id, dims4, maxdims4, hdferr) !output -> dims4
                                                                                        ! maxdims4
                    allocate( F_part(1:dims4(1), 1:dims4(2), 1:dims4(3), 1:dims4(4)) )

                    call h5dread_f(dset_id, type_id, F_part, dims4, hdferr) !Read in array 'F_part'
                    this%F(:,:,:,:,mu) = F_part(:,:,:,:)
        ...
        call h5gopen_f(file_id, grp_name4, grp_id1, hdferr) !Open and existing group, output -> grp_id
            ...
            !................................... PosIndx_old .........................................!
            allocate( this%PosIndx_old(3, 3, Natm, Nbasis) )
            this%PosIndx_old = 0
...
end subroutine set_FC2dat

end module FC2_data_mod
