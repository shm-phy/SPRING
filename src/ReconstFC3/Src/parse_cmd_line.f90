...
subroutine get_command_line(infile, T, write_sBTE)

    implicit none

    character(len=128), parameter       :: expected_cmd_line="./a.out -inp [INPUT FILE] -T [TEMPERATURE] -ShengBTE [T/F]"
    integer, parameter                  :: MAX_CMD_ARG=6

    character(len=*), intent(out)       :: infile
    real(dp), intent(out)               :: T
    logical, intent(out)                :: write_sBTE
    ...
    chk_no_cmd: if ( Narg > 0 .and. Narg <= MAX_CMD_ARG ) then
        ...
        loop_args: do 
            ...
            err_chk: if ( err == 0) then
                ...
                chk_cmd_name: if ( cmd_name == "-inp" ) then
               ...
               else if ( cmd_name == "-ShengBTE" ) then chk_cmd_name
                   ...
                   if ( err == 0 ) then
                       ...
                       Temp_char = cmd_arg
                       dflt_val(3) = .false.
                       read(Temp_char, *) write_sBTE

                       img1_chk5: if ( this_image() == 1 ) then
                           write(*, 700) write_sBTE
                           700 FORMAT('ShengBTE IFC format file writing is set to:', L7)
                       end if img1_chk5
    ...
    dflt_chk_loop: do i = 1, size(dflt_val)
        ...
        outer_if: if ( dflt_val(i) ) then
            ...
            inner_if: if ( i == 1 ) then
                ...
                !-!     write(*, 750) write_sBTE
                !-!     750 FORMAT('ShengBTE IFC format file writing is set to default:', L7)
                !-! end if img1_chk6
...
subroutine print_err_msg(err, nn, cmd_line, expected_cmd_line)
    ...
    subroutine ShowWelcomeBanner()
        ...
        40 FORMAT(10X, "Executable: reconst3.x (Reconstruction of whole set of 3rd-order IFCs from symmetrically reduced set)")
