...
module hdf5_wrap
    ...
    implicit none
    private
    public :: r_force_disp, wFD4_part, wFC4_Reconst, ShengBTE_FC4th
...
subroutine wFD4_part(Mat, force_mua, filename, mu, alpha)
...
end subroutine wFD4_part


subroutine wFC4_Reconst(filename, FC4_Reconst, atmNum, atm_Indx)
    ...
    character(len=*) , intent(in)                           :: filename
    real(dp), dimension(:,:,:,:,:,:,:,:), intent(in)        :: FC4_Reconst
    ...
    dims8 = shape( FC4_Reconst )
    dims7 = dims8(1:7)

    img1_chk: if ( this_image() == 1 ) then
        write(*, 225) filename
        225 FORMAT('Writing FC4 in file: ', A64)
    end if img1_chk
        ...
        call h5fcreate_f(filename, H5F_ACC_TRUNC_F, file_id, h5err)  !output -> file_id
            ...
            call h5gcreate_f(file_id, fc_grp_name, grp_id, h5err) !output -> grp_id
                ...
                mu_loop: do mu = 1, Nbasis
                    ...
                    write(mu_char, frmt) mu
                    dsetname2 = 'FC4_mu'//trim(adjustl(adjustr(mu_char)))

                    call h5screate_simple_f(rank7, dims7, dspace_id2, h5err) !output -> dspace_id2
                    call h5dcreate_f(grp_id, dsetname2, H5T_NATIVE_DOUBLE, dspace_id2, &
                                     dset_id2, h5err) !output -> dset_id2

                    call h5dwrite_f(dset_id2, H5T_NATIVE_DOUBLE, &
                                  & FC4_Reconst(:,:,:,:,:,:,:,mu) , dims7, h5err)
...
end subroutine wFC4_Reconst


    subroutine ShengBTE_FC4th(sys, Temp, FC4_Reconst, atmNum, atm_Indx)
        ...
        character(len=19)                                   :: filename='FORCE_CONSTANTS_4TH'
        ...
        integer                                             :: Nbasis, Natm_tot, Natm_mu, &
                                                             & atom_count
        integer, dimension(3)                               :: Lattice2, Lattice3, Lattice4 
        integer                                             :: basis_indx1, basis_indx2, basis_indx3, &
                                                             & basis_indx4
        integer                                             :: alpha, beta, gama, delta
        integer                                             :: bb, atm2, atm3, atm4, ii
        ...
        5 FORMAT("Writing Fourth-Order IFCs in ShengBTE format: ", A20, '  .Temperature: ', F8.2)
           ...
           & iostat=err, iomsg=err_msg)
            ...
            open_chk: if ( err /= 0 ) then
                ...
                basis_loop2: do bb = 1, Nbasis
                    ...
                    atom2: do atm2 = 1, Natm_mu
                        ...
                        atom3: do atm3 = 1, Natm_mu
                            ...
                            atom4: do atm4 = 1, Natm_mu

                                Lattice4 = atm_Indx(1:3, atm4, bb)
                                basis_indx4 = atm_Indx(4, atm4, bb)

                                Lat_xyz(:, 3) = matmul(sys%latvec, Lattice4)
                                ...
                                write(unit=ui, fmt=50) basis_indx1, basis_indx2, basis_indx3, basis_indx4
                                50 FORMAT( 4I6 )
                                    ...
                                    cart2: do beta = 1, 3
                                        cart3: do gama = 1, 3
                                            cart4: do delta = 1, 3
                                                ...
                                                fc = FC4_Reconst(delta, gama, beta, alpha, atm4, atm3, atm2, bb)

                                                write(unit=ui, fmt=60) alpha, beta, gama, delta, fc
                                                60 FORMAT(3(I2, ' '), I2, '     ', E20.12)
                ...
                check: if ( atom_count /= Natm_tot ) then
                    ...
                    write(*, 14)
                    14 FORMAT("WARNING: atom_count /= Natm_tot ")
