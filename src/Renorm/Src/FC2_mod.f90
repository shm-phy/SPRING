...
contains
    ...
    subroutine FC2_assignment(this, FC2_in)
        ...
        class(FC2type), intent(inout)       :: this
    ...
    subroutine clean_FC2( this )
        ...
        alloc_chk: if ( allocated(this%fC) .and. allocated(this%atmNum) &
                      & .and. allocated(this%atmIndx) ) then

            deallocate( this%fC, stat=istat )
            deallocate( this%atmNum, stat=istat )
            deallocate( this%atmIndx, stat=istat )

        else alloc_chk

            write(*, 444)
            444 FORMAT( 'ERROR in Finalizer: one or more component is already deallocated' )

        end if alloc_chk

    end subroutine clean_FC2
