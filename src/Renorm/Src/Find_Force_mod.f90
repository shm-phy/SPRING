...
contains
    ...
    subroutine CalculateForce(fdfile, FC2_out, force_renorm, force_md)
    
        implicit none
    
        character(len=*)                                                :: fdfile
        type(FC2type), intent(in)                                       :: FC2_out
    
        real(dp), dimension(:), allocatable, intent(out)                :: force_renorm, force_md
        ...
        allocate(force_renorm(product(bound)))
        force_renorm = 0.0_dp
        ...
        mu_loop: do mu = 1, Nbasis
            ...
            alpha_loop: do alpha = 1, 3
                ...
                N2_loop: do N2 = 1, NCb
                    ...
                    Cellnu = FC2_out%atmIndx(:, N2, mu)
                    beta_loop: do beta = 1, 3
                         
                        phi2_dd = FC2_out%fC(1, beta, alpha, N2, mu)
                ...
                zero_chk: if ( any(dabs(force_renorm(strtN:endn)) > cmp_prec) .or. &
                ...
                force_renorm(strtN:endN) = force_mu_alpha
