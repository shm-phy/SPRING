...
module DynaMat
    ...
    public  :: get_phonon_gv, get_phonon
...
subroutine get_phonon_gv(sys, FC2, qpoints, EwaldConst, T, LongEw, &
    ...
    allocate( freq(3*basis, Nq) )
    allocate( nBE(3*basis, Nq) )
    nBE = 0.0_dp
    allocate( Evec(3*basis, 3*basis, Nq) )
    allocate( grp_vel(3, 3*basis, Nq) )
        ...
        do jj = 1, size(neg_check)
            ...
            warning: if ( neg_check(jj) ) then 
                ...
                54 FORMAT("WARNING: Imaginary frequency at q = (", &
                         & 2(F9.4, ', '), F9.4, "). Freqency (omega) = ", F9.4, "THz. Making it to 0.")
...
end subroutine get_phonon_gv


subroutine get_phonon(sys, FC2, qpoints, EwaldConst, T, LongEw, &
                   & freq, nBE, Evec, freq_min, freq_max)
