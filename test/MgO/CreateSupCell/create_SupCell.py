...
class MakeSupCell :
    ...
    def write_siesta_data(self) :
        ...
        accu_float = 3+1+13
        line_format = "{x: <19s}   {y: <19s}   {z: <19s}   {typ: <4s} {Num: <4s} {typlbl: <4s}\n"
        for j in range(Nat) :
            ...
            f2.write(line_format.format(x=str(cord[j, 0])[:accu_float], \
                                        y=str(cord[j, 1])[:accu_float], \
                                        z=str(cord[j, 2])[:accu_float], \
